...
胎面部分的定位意义则有所不同。由于原始高度图中的刀口区域同时受到曲面、热软化和压合作用的影响，直接在原图中给出稳定边界并不容易。因此，系统先通过线性变换与投影，在变换域中把局部几何压缩成更稳定的高灰度结构；随后，再把定位结果反映射回原始图像。第三幅图中的两条红线，正对应代码中的 `head_begin_y` 与 `head_end_y`，也就是抬头范围的上下边界。它们并不是人为画上的辅助说明，而是变换域定位结果回到原图后的最终表达。
...
胎面对接区域的异常在现场是真实存在的，但它们并不总是以肉眼极其显眼的方式出现。这个问题真正困难的地方，恰恰在于异常尺度往往不大，而检测要求却很严格：系统不仅要判断是否存在异常，还要在连续生产条件下，在较小误差容限内对其作出稳定、可比较的区分。

搭接、翘起与虚接等问题，并不是抽象标签，而是真实出现在生产过程中的局部几何异常。它们受到曲面形态、热软化、压合作用以及局部成像条件的共同影响，因此现场检测的难点并不只是“看见某种异常”，而是如何把这些细微但关键的差异转化为稳定、可复核的判别依据。

遗憾的是，当时我没有系统保留更多现场图像；但现存的这几幅图已经足以说明，这类异常在产线中确实存在，而且具有明确的工程判别价值。
